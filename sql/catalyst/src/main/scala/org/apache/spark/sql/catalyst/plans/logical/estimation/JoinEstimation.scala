...
case class InnerOuterEstimation(join: Join) extends Logging {
  ...
  def doEstimate(): Option[Statistics] = join match {
    ...
    case ExtractEquiJoinKeys(joinType, leftKeys, rightKeys, condition, left, right) =>
      ...
      val inputColStats = AttributeMap(
        join.left.statistics.attributeStats.toSeq ++ join.right.statistics.attributeStats.toSeq)
      val attributesWithStat = join.output.filter(a => inputColStats.contains(a))
      ...
      val outputStats: Map[Attribute, ColumnStat] = join.joinType match {
        case LeftOuter =>
          ...
          fromLeft.map(a => (a, inputColStats(a))).toMap ++
            updateColStats(outputRows, fromRight, inputColStats, intersectedStats)
        case RightOuter =>
          // Don't update column stats for attributes from right side.
          updateColStats(outputRows, fromLeft, inputColStats, intersectedStats) ++
            fromRight.map(a => (a, inputColStats(a))).toMap
        case FullOuter =>
          // Don't update column stats for attributes from both sides.
          attributesWithStat.map(a => (a, inputColStats(a))).toMap
        case _ =>
          // Update column stats from both sides for inner or cross join.
          updateColStats(outputRows, attributesWithStat, inputColStats, intersectedStats)
      }
      val outputColStats = AttributeMap(outputStats.toSeq)

      Some(Statistics(
        sizeInBytes = outputRows * getRowSize(join.output, outputColStats),
        rowCount = Some(outputRows),
        attributeStats = outputColStats,
    ...
    case _ =>
      ...
      val outputColStats = AttributeMap(join.output.flatMap(a => inputColStats.get(a).map(a -> _)))
    ...
    if (outputRows == 0) {
      ...
      attributes.foreach { a =>
        ...
        if (joinKeyStats.contains(a)) {
          outputColStats.put(a, joinKeyStats(a))
        } else {
          val oldCS = oldColStats(a)
          ...
          outputColStats.put(a, oldCS.copy(distinctCount = newNdv))
        }
      }
    }
    AttributeMap(outputColStats.toSeq)
  }
...
case class LeftSemiAntiEstimation(join: Join) {
  def doEstimate(): Option[Statistics] = {
    ...
    if (rowCountsExist(join.left)) {
      ...
      val outputColStats = AttributeMap(
        join.output.flatMap(a => join.left.statistics.attributeStats.get(a).map(a -> _)))
