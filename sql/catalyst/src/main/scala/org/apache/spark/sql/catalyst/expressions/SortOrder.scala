 ...
 * `sameOrderExpressions` is a sequence of expressions with the same sort order as the child,
 * E.g. left and right keys from an inner sort merge join.
 */
...
object SortOrder {
  def apply(
     child: Expression,
     direction: SortDirection,
     sameOrderExpressions: Seq[Expression] = Nil): SortOrder = {
