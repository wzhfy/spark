 ...
 * We can add other histograms in the future. But note that estimation logic would become more
 * complicated. Because we would have to deal with computation between different types of
 * histograms in some cases, e.g. for join columns.
 */
trait Histogram {
 ...
 * @param ndv number of distinct values in this bucket
 */
