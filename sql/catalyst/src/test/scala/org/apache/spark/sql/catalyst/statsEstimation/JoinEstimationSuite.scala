...
class JoinEstimationSuite extends StatsEstimationTestBase {
  ...
  private def testJoinEstimationByEHH(
      histogram1: EquiHeightHistogram,
      histogram2: EquiHeightHistogram,
      expectedMin: Double,
      expectedMax: Double,
      expectedNdv: Long,
  ...
  private def generateJoinChild(
    ...
    val t = StatsTestPlan(
      outputList = Seq(col),
      rowCount = (histogram.height * histogram.ehBuckets.length).toLong,
  ...
  private def inferColumnStat(histogram: EquiHeightHistogram): ColumnStat = {
    var ndv = 0L
    for (i <- histogram.ehBuckets.indices) {
      val bucket = histogram.ehBuckets(i)
      if (i == 0 || bucket.hi != histogram.ehBuckets(i - 1).hi) {
        ndv += bucket.ndv
      }
    }
    ColumnStat(distinctCount = ndv, min = Some(histogram.ehBuckets.head.lo),
      max = Some(histogram.ehBuckets.last.hi), nullCount = 0, avgLen = 4, maxLen = 4,
      histogram = Some(histogram))
  }

  test("equi-height histograms: a bucket is contained by another one") {
    val histogram1 = EquiHeightHistogram(height = 300, Seq(
      EquiHeightBucket(lo = 10, hi = 30, ndv = 10), EquiHeightBucket(lo = 30, hi = 60, ndv = 30)))
    val histogram2 = EquiHeightHistogram(height = 100, Seq(
      EquiHeightBucket(lo = 0, hi = 50, ndv = 50), EquiHeightBucket(lo = 50, hi = 100, ndv = 40)))
    // test bucket trimming
    val (t1, h1) = trimBucket(histogram2.ehBuckets(0), height = 100, min = 10, max = 60)
    assert(t1 == EquiHeightBucket(lo = 10, hi = 50, ndv = 40) && h1 == 80)
    val (t2, h2) = trimBucket(histogram2.ehBuckets(1), height = 100, min = 10, max = 60)
    assert(t2 == EquiHeightBucket(lo = 50, hi = 60, ndv = 8) && h2 == 20)
  ...
  test("equi-height histograms: a bucket has only one value") {
    val histogram1 = EquiHeightHistogram(height = 300, Seq(
      EquiHeightBucket(lo = 30, hi = 30, ndv = 1), EquiHeightBucket(lo = 30, hi = 60, ndv = 30)))
    val histogram2 = EquiHeightHistogram(height = 100, Seq(
      EquiHeightBucket(lo = 0, hi = 50, ndv = 50), EquiHeightBucket(lo = 50, hi = 100, ndv = 40)))
    // test bucket trimming
    val (t1, h1) = trimBucket(histogram2.ehBuckets(0), height = 100, min = 30, max = 60)
    assert(t1 == EquiHeightBucket(lo = 30, hi = 50, ndv = 20) && h1 == 40)
    val (t2, h2) = trimBucket(histogram2.ehBuckets(1), height = 100, min = 30, max = 60)
    assert(t2 ==EquiHeightBucket(lo = 50, hi = 60, ndv = 8) && h2 == 20)
  ...
  test("equi-height histograms: a bucket has only one value after trimming") {
    val histogram1 = EquiHeightHistogram(height = 300, Seq(
      EquiHeightBucket(lo = 50, hi = 60, ndv = 10), EquiHeightBucket(lo = 60, hi = 75, ndv = 3)))
    val histogram2 = EquiHeightHistogram(height = 100, Seq(
      EquiHeightBucket(lo = 0, hi = 50, ndv = 50), EquiHeightBucket(lo = 50, hi = 100, ndv = 40)))
    // test bucket trimming
    val (t1, h1) = trimBucket(histogram2.ehBuckets(0), height = 100, min = 50, max = 75)
    assert(t1 == EquiHeightBucket(lo = 50, hi = 50, ndv = 1) && h1 == 2)
    val (t2, h2) = trimBucket(histogram2.ehBuckets(1), height = 100, min = 50, max = 75)
    assert(t2 == EquiHeightBucket(lo = 50, hi = 75, ndv = 20) && h2 == 50)
  ...
  test("equi-height histograms: skip and trim buckets by min/max") {
    // This case can happen when estimating join after a filter.
    val histogram1 = EquiHeightHistogram(height = 300, Seq(
      EquiHeightBucket(lo = 1, hi = 30, ndv = 10),
      EquiHeightBucket(lo = 30, hi = 60, ndv = 30),
      EquiHeightBucket(lo = 60, hi = 90, ndv = 6),
      EquiHeightBucket(lo = 90, hi = 200, ndv = 30)))
    val histogram2 = EquiHeightHistogram(height = 100, Seq(
      EquiHeightBucket(lo = -50, hi = 0, ndv = 50),
      EquiHeightBucket(lo = 0, hi = 50, ndv = 50),
      EquiHeightBucket(lo = 50, hi = 100, ndv = 40),
      EquiHeightBucket(lo = 100, hi = 150, ndv = 40)))

    // test bucket trimming
    val (t1, h1) = trimBucket(histogram1.ehBuckets(1), height = 300, min = 50, max = 75)
    assert(t1 == EquiHeightBucket(lo = 50, hi = 60, ndv = 10) && h1 == 100)
    val (t2, h2) = trimBucket(histogram1.ehBuckets(2), height = 300, min = 50, max = 75)
    assert(t2 == EquiHeightBucket(lo = 60, hi = 75, ndv = 3) && h2 == 150)
    val (t3, h3) = trimBucket(histogram2.ehBuckets(1), height = 100, min = 50, max = 75)
    assert(t3 == EquiHeightBucket(lo = 50, hi = 50, ndv = 1) && h3 == 2)
    val (t4, h4) = trimBucket(histogram2.ehBuckets(2), height = 100, min = 50, max = 75)
    assert(t4 == EquiHeightBucket(lo = 50, hi = 75, ndv = 20) && h4 == 50)
